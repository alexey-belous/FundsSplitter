...
- `/pay 100` - Adds payment of particular user (Example: `/pay 10` adds 10 UAH and split them between everyone, `/pay 10 @user1 @user2` adds 10 UAH and split them between @user1 and @user2).
...
- `/pay 100` - Добавляет платеж пользователя, отправившего сообщение (Пример: `/pay 10` добавляет 10 грн и разделяет их между всеми участниками группы, `/pay 10 @user1 @user2` добавляет 10 грн и разделяет их между пользователями @user1 и @user2).
